...
class Record(TimestampMixin, Base):
    ...
    # CORRECCIÓN: Integer estándar (agnóstico de BD) para FKs de catálogo
    record_type_id = Column(Integer, ForeignKey("record_types.id"), nullable=False)
    status_id      = Column(Integer, ForeignKey("record_statuses.id"), nullable=False)
    ...
    # active_version_id no tiene FK explícita en SQL (referencia circular manejada en app)
    active_version_id  = Column(String(36), nullable=True)
    # CORRECCIÓN: Integer estándar (agnóstico de BD). Valor siempre >= 0.
    latest_version_num = Column(Integer, nullable=False, default=0)
