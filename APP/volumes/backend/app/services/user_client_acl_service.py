...
#
# Gestiona los permisos granulares usuario ↔ cliente (read / edit / owner).
# Presupone que el usuario ya existe en user_clients para ese cliente.
# Para la asignación básica (is_active), ver user_clients_service.
#
...
def _get_or_404(db: Session, user_id: str, client_id: str) -> UserClientAcl:
    ...
    if not obj:
        raise HTTPException(status_code=404, detail="USER_CLIENT_ACL_NOT_FOUND")
    return obj


def _build_response_dict(obj: UserClientAcl) -> dict[str, Any]:
    return {
...
def get_user_client_acl(db: Session, user_id: str, client_id: str) -> dict[str, Any]:
    return _build_response_dict(_get_or_404(db, user_id, client_id))


def list_user_client_acls(db: Session, filters: UserClientAclFilterRequest) -> dict[str, Any]:
...
def create_user_client_acl(
    ...
    if existing:
        raise HTTPException(status_code=409, detail="USER_CLIENT_ACL_ALREADY_EXISTS")
