...
def _check_unique_record_num(db: Session, record_id: str, version_num: int, exclude_id: str | None = None) -> None:
    q = (
        db.query(RecordVersion)
        .filter(RecordVersion.record_id == record_id)
        .filter(RecordVersion.version_num == version_num)
        .filter(RecordVersion.deleted_at.is_(None))
    )
    if exclude_id:
...
def list_record_versions(db: Session, filters: RecordVersionFilterRequest) -> dict:
    ...
    if filters.published_by:
        q = q.filter(RecordVersion.published_by == filters.published_by)
...
def create_record_version(db: Session, body: RecordVersionCreateRequest, created_by_id: str) -> dict:
    _check_unique_record_num(db, body.record_id, body.version_num, exclude_id=None)
...
def update_record_version(db: Session, id: str, body: RecordVersionUpdateRequest, updated_by_id: str) -> dict:
    obj = _get_or_404(db, id)
    ...
    if body.published_by is not None:
        obj.published_by = body.published_by
