/**
 * ProjectFilters.jsx
 * Componente de filtros para el módulo de proyectos
 * Ajustado visualmente para calzar con ClientFilters
 */
...
// Subcomponente: FilterDropdown (idéntico al patrón ClientFilters)
...
// Subcomponente: FilterField (alineado a ClientFilters, con search con icono interno)
...
const ProjectFilters = ({ filters, onFilterChange, onClearFilters, onApplyFilters, data }) => {
  ...
  // Options (puedes reemplazar clientOptions por derivación desde `data`)
  const statusOptions = [
  ...
  return (
    <div className="bg-surface shadow-card rounded-2xl p-6 mb-6 border border-secondary-200 dark:border-secondary-700/60 dark:ring-1 dark:ring-white/5 transition-theme">
      {/* Header (idéntico a ClientFilters) */}
      ...
      {/* Filters Grid (misma grilla que ClientFilters) */}
      {filtersExpanded && (
