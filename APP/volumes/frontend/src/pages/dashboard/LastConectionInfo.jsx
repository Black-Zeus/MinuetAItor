 ...
 *
 * Fuente de verdad: sessionStore.connections
 *  { active: { ts, device, location, ip_v4, ip_v6 }, last: [...] }
 */
...
const InfoPill = ({ icon, label, value, noWrap = false }) => (
  <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-900/40 border border-gray-100 dark:border-gray-700/50 transition-theme">
    ...
    <div className="min-w-0 flex-1">
      ...
      <p
        className={[
          `text-sm font-semibold ${TXT_TITLE} transition-theme leading-tight`,
          noWrap ? "truncate" : "whitespace-normal break-words",
        ].join(" ")}
        title={value || "—"}
      >
        {value || "—"}
...
// ─── Info pill multi-línea (IP v4/v6) ────────────────────────────────────────
        ...
        <div className="mt-1 space-y-1">
          {lines.map((x, idx) => (
            <div key={`${x.label}-${idx}`} className="flex items-baseline gap-2 min-w-0">
              ...
              <span
                className={`text-sm font-semibold ${TXT_TITLE} truncate transition-theme`}
                title={x.value}
              >
                {x.value}
              </span>
...
const ConnectionRow = ({ conn }) => {
  const ipLines = buildIpLines({
    ip_v4: conn?.ip_v4 ?? conn?.ip ?? "",
    ip_v6: conn?.ip_v6 ?? "",
  });
  ...
  return (
    <div className="flex items-start justify-between gap-4 p-3 rounded-xl bg-white dark:bg-gray-900/20 border border-gray-200 dark:border-gray-700/50 transition-theme">
      <div className="min-w-0">
        ...
        <p className={`text-xs ${TXT_META} transition-theme mt-0.5 whitespace-normal break-words`}>
          ...
          <div className="space-y-1">
            {ipLines.map((x, idx) => (
              <div key={`${x.label}-${idx}`} className="flex items-baseline gap-2 justify-end">
                <span className="text-[11px] font-semibold text-gray-500 dark:text-gray-400">{x.label}</span>
                <span className={`text-xs font-semibold ${TXT_TITLE} transition-theme`}>{x.value}</span>
              </div>
            ))}
          </div>
        ) : (
...
const LastConectionInfo = () => {
  ...
  // Normalizar al mismo "session view model" que usaba la UI anterior
  const session = useMemo(() => {
    const active = connections?.active ?? null;
    const last   = Array.isArray(connections?.last) ? connections.last : [];

    return {
      isLoggedIn:       isLoaded,
      activeConnection: active ? toUiConnection(active) : null,
      lastConnections:  last.map(toUiConnection),
      // fallbacks compat para resolveCurrentConnection
      lastAccess:   active?.ts       ?? null,
      lastDevice:   active?.device   ?? null,
      lastLocation: active?.location ?? null,
      lastIp:       active?.ip_v4    ?? "",
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-6 transition-theme">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
        <InfoPill icon="FaCalendarAlt" label="Fecha y hora"  value={formatDateTimeTechnical(current.ts)} />
        <InfoPill icon="FaDesktop"     label="Dispositivo"   value={current.device} />
        <InfoPill icon="FaLocationDot" label="Ubicación"     value={current.location} />
        <InfoPillMulti icon="FaNetworkWired" label="Dirección IP" lines={ipLines} />
      </div>
      ...
      <div className="mt-5 border border-gray-200 dark:border-gray-700/60 rounded-xl overflow-hidden transition-theme">
        <button
          ...
          className="w-full flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-900/40 hover:bg-gray-100 dark:hover:bg-gray-900/60 transition-theme cursor-pointer select-none"
        >
          <span className={`text-sm font-bold ${TXT_TITLE} flex items-center gap-2 transition-theme`}>
            <Icon name="FaClockRotateLeft" className="text-primary-500 dark:text-primary-400 w-3.5 h-3.5" />
            ...
            {lastXConnections.length ? (
              <div className="space-y-2">
                {lastXConnections.map((c, idx) => (
                  <ConnectionRow key={`${c?.ts ?? idx}-${idx}`} conn={c} />
                ))}
              </div>
            ) : (
              <p className={`text-sm ${TXT_BODY} transition-theme`}>Sin historial de conexiones.</p>
            )}
          </div>
        )}
      </div>
