...
import { PDF_TEMPLATE_OPTIONS, getPdfTemplateMeta } from '@/constants/pdfTemplates';
...
export const TemplateSelector = ({ recordId, selectedId, onChange, isReadOnly }) => {
...
export const SheetToggleCard = ({ title, description, icon, enabled, onToggle, children, isReadOnly = false }) => (
...
const CoverPageConfig = () => {
  const { pdfFormat, updateCoverPage } = useMinuteEditorStore();
...
const SummarySheetConfig = () => {
  const { meetingInfo, metadataLocked, timeline } = useMinuteEditorStore();
